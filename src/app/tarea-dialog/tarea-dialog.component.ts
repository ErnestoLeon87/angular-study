...
export class TareaDialogComponent implements OnInit, OnDestroy {
  ...
  crearTarea(): void {
    ...
    this.subscription = this.tareaSvc
      .addTarea$(this.formTarea.value)
      .pipe(finalize(() => this.spinner.desactiveSpinner()))
      .subscribe({
        next: (dat) =>
          this._snackBar.open(dat.titulo, 'se a añadido...', {
            duration: 2000,
          }),
  ...
  editarTarea(): void {
    this.subscription = this.tareaSvc
      .editTarea$(this.getTareaReadyToSave(this.tareaEdit.id))
      .subscribe({
        next: (dat) =>
